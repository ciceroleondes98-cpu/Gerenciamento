...
# 3. Conexão com o Google Sheets através do secrets do Streamlit
try:
...
if not conexao_ok:
    st.error("❌ Erro de conexão. Verifique se o arquivo secrets.toml foi criado corretamente no Streamlit com o link da sua planilha.")
else:
    # Captura os parâmetros operacionais da aba 'config'
    if not dados_config.empty:
        val_saldo = dados_config['saldo_inicial'].iloc[0]
        val_meta_f = dados_config['meta_final'].iloc[0]
        val_meta_d = dados_config['meta_diaria'].iloc[0]
        val_dias = dados_config['qtd_dias'].iloc[0]
    else:
    ...
    with col_banca1:
        saldo_banca_inicial = st.number_input("Saldo Inicial (R$):", min_value=0.0, value=float(val_saldo), step=10.0)
    with col_banca2:
        meta_final = st.number_input("Meta Final Geral (R$):", min_value=1.0, value=float(val_meta_f), step=50.0)
    with col_banca3:
        meta_diaria = st.number_input("Meta Diária (R$):", min_value=0.0, value=float(val_meta_d), step=1.0)
    with col_banca4:
        quantidade_dias = st.number_input("Qtd de Dias:", min_value=1, max_value=365, value=int(val_dias), step=1)

    # Link direto para facilitar o acesso visual do usuário para salvar/atualizar manual se necessário
    st.markdown(f"[🔗 Abrir Planilha Google Sheets para salvar alterações]({url_planilha})")

    # Estruturação e cálculo da tabela dinâmica de datas (Junho/2026)
    datas_fixas = [(datetime.date(2026, 6, 1) + datetime.timedelta(days=i)).strftime('%d/%m/%Y') for i in range(quantidade_dias)]
    
    # Mapeia rendimentos salvos na planilha
    dict_rendimentos = {}
    if not dados_rendimentos.empty and 'Data' in dados_rendimentos.columns and 'Rendimento' in dados_rendimentos.columns:
        dict_rendimentos = dict(zip(dados_rendimentos['Data'].astype(str), dados_rendimentos['Rendimento'].astype(float)))
    ...
    for idx, data in enumerate(datas_fixas):
        ...
        rend = dict_rendimentos.get(data, 0.0)
    ...
    # Filtragem de segurança para indicadores realistas
    ...
    # Bloco visual - Resumo da Operação
    st.markdown(f"""
    ...
    with tab1:
        ...
        st.info("💡 Abra a planilha pelo link acima para colar os novos dados diretamente nas respectivas abas se desejar atualizar instantaneamente!")
    ...
    with tab3:
        ...
        if not df_preenchidos.empty:
            ...
            st.warning("Insira lançamentos na planilha para gerar as barras de evolução gráfica.")
